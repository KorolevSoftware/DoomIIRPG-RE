...
- [ ] Фаза 4. Игровые сущности: Player, Entity, инвентарь, оружие, combat.
      ...
      - [x] Подбор предметов (touchedItem, give): domain/game/ItemPickup +
        Game::touchTile. Спавн ET_ITEM-сущностей в loadEntities (форс-кадра
        оружия 0x200 для тайлов 1..12), подбор по занятости целевого тайла в
        finishMovement (после executeTile, до advanceTurn, ход НЕ тратится),
        гранты по классам с легасевыми роллами, removeEntity + статик-скрипт 11.
        Player::give доведён до легаси: quiet, авто-экипировка первого
        получения, sentry-bot pre-step, bottled→holy water.
        Спека: docs/architecture/specs/2026-08-29-world-item-pickup.md, ADR-0014.
        НЕ портировано: звук 1054, ET_MONSTERBLOCK_ITEM (0 def'ов в данных),
        dropped-item плумбинг, help-подсказки, ET_ENV_DAMAGE.
      - [x] Подбор с полок по действию: `EV_GIVEITEM` режим 0 (два байта операнда =
        big-endian индекс СПРАЙТА) → `findEntityBySprite` → `ItemPickup::touched`.
        Точка входа — executeTile передней клетки с флагом TRIGGER
        (PlayerActions.cpp), ход ТРАТИТСЯ (в отличие от подбора шагом).
        Двойной подбор (шагом + действием) возможен — так в оригинале, guard
        не добавляли. Спека 2026-08-30-blocking-crates-shelf-pickup.md §G1.
      - [x] Полный спавн сущностей (ADR-0015): сущность на КАЖДЫЙ спрайт с def
        + fallback для def-less спрайт-стен (0x800000, заимствование find(12/13,0)),
        nudge тайла привязки для ориентированных спрайтов, ветки initspawn по
        семействам, скрытые спрайты спавнятся но не линкуются. Блокировка декора,
        ящиков и спрайт-стен (было: проходились насквозь, 86 объектов на map00).
        map00: 191 сущность, 193/275 слотов, 35/80 монстров, 20 сдвигов. §G2.
      - [x] Ящики: блокируют, открываются действием с дистанции 1 клетка
        (немедленный unlink → клетка проходима сразу, ещё до анимации),
        анимация 4 кадра по 200 мс с залипанием на кадре 3, содержимое через
        EV_GIVELOOT (мгновенная выдача + диалог стиля 4, заголовок
        "<имя> Con-tents:", ровно один ход на закрытии). §G3/§G4, ADR-0016.
        Неуязвимы для оружия (маска попадания 0) — как в оригинале.
      - [x] Ветка оружия G1/G3 (спека 2026-08-31-weapon-branch.md, ADR-0017/0018):
        бензопила наносит урон — заглушка отвергала её как «оружие со снарядом»
        (`projType == -1`); путь -1 реализован как настоящая ветка `default:`
        машины снарядов (`launchProjectile`/`updateProjectile`, ADR-0017), а не
        как отдельный ближний бой. Guard стал `proj > 0` — точен для оружия
        игрока (6 отвергнутых видов реально требуют пула снарядов, 0 ошибочных).
        Диспетчеры `Game::entityPain`/`entityDied` (ADR-0018) — до них урон по
        всему, что не монстр, молча не работал. Мебель (121/135) рубится только
        пилой (+5 XP, стр. 89), ящики неуязвимы (маска 0), баррикада (178)
        ломается и перестаёт блокировать. Починен потерянный при портировании
        `else if`: промах пилой больше не будит монстра.
        НЕ сделано: G2 (рост силы, тряска), G4 (гибы трупов), G5 (настоящие
        снаряды: святая вода, плазма, ракеты, BFG, куб душ, брошенный предмет).
