...
_Last updated: 2026-08-30 (evening)_
...
- **Known bugs (deferred):**
  ...
  - **Расхождения с оригиналом, найденные ревьюерами 2026-08-30 (низкий приоритет,
    ...
    2. `ET_DOOR` получает `kInfoActive` (`Game.cpp` ветка двери), которого в
       ...
       промах. НАБЛЮДАЕМО.
    3. Первый пре-пасс `loadEntities` не портирует две легасевые инъекции
       анимации: тайл 156 (`n7=2`, `|= 0x80200`) и тайл 236 (`n7=3`,
       `|= 0x80300` + `S_RENDERMODE = 3`), `src/Game.cpp:383-393`. У нас только
       130/136/234 → два вида объектов статичны там, где должны анимироваться.
       НАБЛЮДАЕМО.
