...
- **Режимы смешивания спрайтов (blend modes) — G1/G2 сделаны, ревью НЕ проходило.**
  ...
  - **ДОЛГ (верность оригиналу, найден 2026-09-01):** наша сортировка спрайтов в
    листе неустойчивая (`if (leafDepth_[b] > leafDepth_[a]) swap`,
    `new_src/render/World3D.cpp:1613-1621`), тогда как легаси при РАВНЫХ ключах
    даёт строгое возрастание индекса спрайта. Значит любые две равноключевые пары
    у нас могут переставляться непредсказуемо, не только огонь/нагар. Доказано
    численно логом: в листе 214 группа key=760 вышла как 31,32,30, а пара key=762
    как 29,28. Чинить отдельно от отклонения №2, иначе одно замаскирует другое.
